...
class TritonPythonModel:
    ...
    def initialize(self, args):
        ...
        """
        print(f'pipeline will construct, show lmdeploy version: {lmdeploy.__version__}')
        # You must parse model_config. JSON string is not parsed here
        pwd = os.path.abspath(os.path.dirname(__file__))
        self.max_batch_size = json.loads(args['model_config'])['max_batch_size']
        self.ckpt_path = f'{pwd}/checkpoint/InternVL2-26B-Quant'
        self.gen_config = GenerationConfig(max_new_tokens=1024)
    ...
    def evaluate(self, query, image):
        ...
        print("will pipeline show prompts size: ", len(prompts))
        # import pdb;pdb.set_trace()
        responses = self.pipeline(prompts, gen_config=self.gen_config) #, do_preprocess=False)
        print(f'pipeline output: {responses}')
    ...
    def execute(self, requests):
        ...
        # import pdb;pdb.set_trace()
        outputs = np.concatenate(np.expand_dims(outputs, 0))
        out_tensor_0 = pb_utils.Tensor("response", outputs.astype(np.string_))
